...
if "LinkKeys" in pl:
    ...
    for adapter in pl["LinkKeys"].keys():
        ...
        for device in pl["LinkKeys"][adapter].keys():
            f.write('\r\n"'+device.replace("-", "")+'"=hex:' +
                    convertToWinRep(pl["LinkKeys"][adapter][device].data.hex()))
else:
...
if "SMPDistributionKeys" in pl:
    ...
    for adapter in pl["SMPDistributionKeys"].keys():
        ...
        for device in pl["SMPDistributionKeys"][adapter].keys():
            ...
            dev += '\r\n"EDIV"=dword:' + convertToWinRep2(
                pl["SMPDistributionKeys"][adapter][device]["EDIV"].data.hex(
                ).ljust(8, '0'))

            # Identity Resolving Key (IRK)
            # 128-bit key used to generate and resolve random address.
            dev += '\r\n"IRK"=hex:' + \
                convertToWinRep(pl["SMPDistributionKeys"]
                                [adapter][device]["IRK"].data.hex())

            # Device Address
            # 48-bit Address of the connected device
            dev += '\r\n"Address"=hex(b):' + convertToWinRep(
                pl["SMPDistributionKeys"][adapter][device]["Address"].data.hex(
                ).rjust(16, '0'))
